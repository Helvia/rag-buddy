...
Use $ python -m python.scripts.tc_proxy_client --help to display the script's usage information.

To execute the script use $ python -m python.scripts.tc_proxy_client --cache-control no-cache no-store
and then enter the prompt e.g. "Credit card not valid anymore?" .