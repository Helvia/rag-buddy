...
async def call_RAG(system_intro, articles, user_message, cache_control, stream, proxy):
...
async def test_rag_completion_async_stream(
    prompt: str, articles: List, cache_control: str, proxy: str
):
    user_message = prompt
    system_intro = (
    ...
    completion, cache_hit = await call_RAG(
        system_intro, articles, user_message, cache_control, True, proxy
    )
...
async def test_rag_completion_async(
    ...
    completion, cache_hit = await call_RAG(
        system_intro, articles, user_message, cache_control, False, proxy
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="local",
        help="Set the proxy environment (local, dev, prod)",
    )
