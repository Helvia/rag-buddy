...
cache_status = completion.headers.get("Helvia-RAG-Buddy-Cache-Status")
cache_hit = int(cache_status) if cache_status not in ["None", None] else None

if cache_hit is not None:
        print(f"Cache hit: {cache_hit}")
api_return_dict = json.loads(completion.content.decode())
api_message_dict = api_return_dict.get('choices')[0].get('message')


# Read the response
print(api_message_dict.get('content'))
